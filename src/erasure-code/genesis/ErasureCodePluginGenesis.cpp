...
#include "ErasureCodePluginGenesis.h"
#include "genesis_init.h"
...
int ErasureCodePluginGenesis::factory(const std::string& directory,
				       ceph::ErasureCodeProfile &profile,
				       ceph::ErasureCodeInterfaceRef *erasure_code,
				       std::ostream *ss) {
    ErasureCodeGenesis *interface;
    std::string t;
...
int __erasure_code_init(char *plugin_name, char *directory)
{
  ...
  int w[] = { 4, 8, 16, 32 };
  int r = genesis_init(4, w);
  if (r) {
    return -r;
  }
  return instance.add(plugin_name, new ErasureCodePluginGenesis());
}
